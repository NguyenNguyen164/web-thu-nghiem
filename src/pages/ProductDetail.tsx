...
import {
  ...
  useToast, Spinner, Flex, useBreakpointValue, IconButton, SimpleGrid
  ...
  if (!product) {
    return (
      <Container maxW="container.xl" py={12} textAlign="center">
        ...
        <Button leftIcon={<FiArrowLeft />} colorScheme="blue" onClick={() => navigate('/products')}>
    ...
    <Box bg="gray.50" minH="100vh">
      <Container maxW="container.xl" py={8}>
          ...
          <Stack direction={{ base: 'column', lg: 'row' }} spacing={8} p={{ base: 4, md: 8 }}>
            ...
            <Box flex={1} pl={{ lg: 4 }}>
              ...
              <Box mb={8}>
                ...
                {product.description ? (
                  <Text color="gray.700" lineHeight="tall">{product.description}</Text>
                ) : (
              ...
              {product.attributes && (
                <Box mb={8}>
                  ...
                  <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                    ...
                    {product.attributes.dimensions_cm && Object.entries(product.attributes.dimensions_cm).map(([key, value]) => (
                      <Box key={key}>
                        <Text fontWeight="medium" color="gray.600" textTransform="capitalize">
                          {String(key).replace(/_/g, ' ')}
                        </Text>
                        <Text color="gray.800">{value} cm</Text>
                      </Box>
                    ))}
                  </SimpleGrid>
                </Box>
        ...
        {relatedProducts.length > 0 && (
          <Box mb={8}>
            <Heading size="md" mb={6} color="gray.800">Sản phẩm liên quan</Heading>
            <SimpleGrid columns={{ base: 2, md: 4 }} spacing={6}>
              {relatedProducts.map((relatedProduct) => (
                <Box key={relatedProduct.id} as={Link} to={`/san-pham/${relatedProduct.id}`} _hover={{ textDecoration: 'none' }}>
                  <Box
                    bg="white"
                    borderRadius="lg"
                    overflow="hidden"
                  ...
                  >
                    <Box flex={1} p={4}>
                      ...
                      <Text fontWeight="bold" color="blue.600">{formatPrice(relatedProduct.price)}</Text>
        ...
        <Box textAlign="center" mt={8} mb={4}>
          <Button variant="ghost" leftIcon={<FiArrowUp />} onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            Lên đầu trang
          </Button>
        </Box>
      </Container>
